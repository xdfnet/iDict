...
配置文件保存时应保持字段顺序：`provider`、`baseURL`、`apiKey`、`model`、`systemPrompt`、`userPromptTemplate`、`timeoutSeconds`、`speechEnabled`、`speechCommandPath`。不要把路径写死为 `/Users/admin`，应使用当前用户 Home。
