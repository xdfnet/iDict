...
一款简洁高效的 macOS 菜单栏应用，集成快速翻译、媒体远程控制、智能锁屏登录等功能。通过 `Cmd+D` 快速翻译选中文本，或使用手机远程控制 Mac 的媒体播放、应用开关等。
...
- **多服务支持** - 腾讯翻译、Google、Microsoft、DeepL 四种翻译引擎可选
- **实时检测** - 自动检测服务状态，智能切换可用引擎
- **图形化配置** - 内置设置界面，无需手动配置环境变量
...
- **无边框窗口** - 简洁美观的翻译结果展示
- **拖拽移动** - 自由调整翻译窗口位置
- **快捷键支持** - `ESC` / `Cmd+W` 快速关闭
- **内容自适应** - 窗口大小随翻译内容自动调整

### 🔒 安全隐私
- **本地处理** - 所有操作本地完成，不存储翻译历史
- **权限管理** - 智能请求必要的系统权限
- **安全存储** - API 密钥和密码使用 Keychain 安全存储
