...
手机浏览器 → http://<mac-ip>:8888
  → MediaHTTPServer.processRequest()
    → handleAPI()
      ...
      → handleAPIAction() 分派
        ├── status    → MediaController.hasMediaAppRunning()
        ├── play      → MediaRemoteBridge.sendCommand(.play)
        ├── pause     → MediaRemoteBridge.sendCommand(.pause)
        ├── next/prev → MediaRemoteBridge.sendCommand(.nextTrack/.previousTrack)
        ├── volume/mute → CGEvent 模拟媒体按键
...
```
MediaController.play() / pause() / nextTrack() / previousTrack()
  │
  ├─ 抖音或汽水音乐未运行 → 跳过
  │
  └─ 抖音或汽水音乐在运行
      └─ MediaRemoteBridge.sendCommand(.play/.pause/.nextTrack/.previousTrack)
          → MRMediaRemoteSendCommand → 系统路由到当前媒体会话
```

音量和方向键仍走 CGEvent 模拟按键，MediaRemote 无对应命令。
...
`status` `lock_status` `status_douyin` `status_qishui` `play` `pause` `next` `prev`

**需辅助功能权限**：`volumeup` `volumedown` `mute` `arrowup` `arrowdown` `lock` `toggle_douyin` `toggle_qishui`

> MediaRemote 命令（play/pause/next/prev）走系统媒体路由，无需辅助功能权限。CGEvent 模拟和 App 管理才需要。
...
通过 `dlopen` + `dlsym` 动态加载私有框架 `/System/Library/PrivateFrameworks/MediaRemote.framework/MediaRemote`，避免编译期符号依赖。
...
| `kMRPause` | 1 | 精确暂停 |
| `kMRNextTrack` | 4 | 下一曲 |
| `kMRPreviousTrack` | 5 | 上一曲 |

### 局限

- 仅 `SendCommand` 可用，查询类 API（`GetNowPlayingClient`、`GetNowPlayingApplicationIsPlaying`）因 Electron 应用不注册 NowPlaying client 而无效
- 音量控制不走媒体会话路由，仍用 CGEvent
