...
let sizes = [16, 32, 128, 256, 512] // macOS AppIcon 标准 10 个文件（含 @2x）
...
func render(_ px: Int) -> NSImage {
    let s = CGFloat(px)
    // 用 1x 位图绘制，避免 Retina 下 tiff 输出双倍尺寸
    ...
    // translate 符号，饱和天蓝，居中约 70%
    let cfg = NSImage.SymbolConfiguration(pointSize: s * 0.70, weight: .semibold)
    if let sym = NSImage(systemSymbolName: "translate", accessibilityDescription: nil)?
        ...
        let scale = (s * 0.70) / max(sw, sh)
        let dw = sw * scale, dh = sh * scale
        let dx = (s - dw)/2, dy = (s - dh)/2
